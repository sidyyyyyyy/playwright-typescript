...
test('HealthPlan Agent - Send 3 questions sequentially and collect responses', async ({ page }) => {
  console.log('🏥 HealthPlan Agent Test: Sending 3 questions sequentially...');
  ...
  await page.screenshot({ 
    path: 'reports/healthplan-initial-state.png',
    fullPage: true 
  });
  ...
  for (let i = 0; i < healthQuestions.length; i++) {
    ...
    try {
      ...
      if (await chatInput.isVisible({ timeout: 10000 })) {
        ...
        await page.screenshot({ 
          path: `reports/healthplan-question-${i + 1}-before-send.png`,
          fullPage: true 
        });
        ...
        for (let waitCount = 1; waitCount <= 25; waitCount++) {
          ...
          if (waitCount <= 5 || waitCount % 5 === 0) {
            await page.screenshot({ 
              path: `reports/healthplan-question-${i + 1}-wait-${waitCount}.png`,
              fullPage: true 
            });
          }
  ...
  console.log('\n📋 ===== HEALTHPLAN AGENT COMPREHENSIVE TEST REPORT =====');
  ...
  await page.screenshot({ 
    path: 'reports/healthplan-3questions-final.png',
    fullPage: true 
  });
  console.log('📸 Final test screenshot saved');
  
  console.log('\n✅ HealthPlan Agent 3-Question Test completed!');
