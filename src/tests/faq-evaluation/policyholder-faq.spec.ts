...
test('PolicyHolder Agent - FAQ Evaluation Test', async ({ page }) => {
  console.log('🛡️ PolicyHolder Agent FAQ Evaluation Test Starting...');
  ...
  await page.screenshot({ 
    path: 'reports/faq-evaluation/policyholder-initial.png',
    fullPage: true 
  });
  
  // Define PolicyHolder-specific FAQ questions
  const insuranceFAQQuestions = [
    {
      question: "What does my auto insurance policy cover in case of an accident?",
      category: "Auto Coverage",
      expectedKeywords: ["auto", "insurance", "policy", "cover", "accident", "coverage"],
      difficulty: "Basic"
    },
    {
      question: "How do I file a claim for property damage?",
      category: "Claims Process",
      expectedKeywords: ["file", "claim", "property", "damage", "process", "submit"],
      difficulty: "Intermediate"
    },
    {
      question: "What is the timeline for processing my insurance claim?",
      category: "Claim Timeline",
      expectedKeywords: ["timeline", "processing", "claim", "time", "duration", "period"],
      difficulty: "Intermediate"
    },
    {
      question: "Can you explain my billing statement and premium calculations?",
      category: "Billing & Premiums",
      expectedKeywords: ["billing", "statement", "premium", "calculations", "cost", "payment"],
      difficulty: "Intermediate"
    },
    {
      question: "What endorsements can I add to my policy for additional coverage?",
      category: "Policy Endorsements",
      expectedKeywords: ["endorsements", "policy", "additional", "coverage", "add", "enhance"],
      difficulty: "Advanced"
    }
  ];
  
  console.log('✅ Prepared 5 PolicyHolder FAQ questions for evaluation');
  console.log('📹 Video recording is active - capturing entire interaction');
  console.log('🔍 FAQ Evaluation Mode: Testing insurance domain knowledge and conversation context');
  
  // Store all responses and analysis
  const testResults = [];
  
  // Send each FAQ question and collect responses
  for (let i = 0; i < insuranceFAQQuestions.length; i++) {
    const { question, category, expectedKeywords, difficulty } = insuranceFAQQuestions[i];
    ...
    try {
      ...
      if (await chatInput.isVisible({ timeout: 15000 })) {
        ...
        await page.screenshot({ 
          path: `reports/faq-evaluation/policyholder-before-question-${i + 1}.png`,
          fullPage: true 
        });
        ...
        await page.screenshot({ 
          path: `reports/faq-evaluation/policyholder-question-${i + 1}-before-send.png`,
          fullPage: true 
        });
        ...
        for (let waitCount = 1; waitCount <= 30; waitCount++) {
          ...
          if (waitCount <= 5 || waitCount % 5 === 0) {
            await page.screenshot({ 
              path: `reports/faq-evaluation/policyholder-question-${i + 1}-wait-${waitCount}.png`,
              fullPage: true 
            });
          }
        ...
        const analysis = analyzeInsuranceFAQResponse(question, responseText, expectedKeywords, category);
        ...
        if (i < insuranceFAQQuestions.length - 1) {
  ...
  console.log('\n📋 ===== POLICYHOLDER AGENT FAQ EVALUATION REPORT =====');
  ...
  console.log('🔍 FAQ Evaluation: Testing insurance domain knowledge and conversation context');
  ...
  await page.screenshot({ 
    path: 'reports/faq-evaluation/policyholder-faq-final.png',
    fullPage: true 
  });
  console.log('📸 Final FAQ evaluation screenshot saved');
  
  console.log('\n✅ PolicyHolder Agent FAQ Evaluation Test completed!');
  console.log('📹 Check the test results folder for the recorded video file');
  console.log('🔍 FAQ Evaluation: Insurance domain knowledge and conversation context tested');
});

// Helper function to analyze PolicyHolder FAQ responses
function analyzeInsuranceFAQResponse(question: string, response: string, expectedKeywords: string[], category: string) {
  ...
  // Calculate domain accuracy (how well it answers insurance questions)
  const insuranceDomainTerms = [
    'policy', 'coverage', 'claim', 'endorsement', 'premium', 'deductible',
    'timeline', 'process', 'procedure', 'documentation', 'requirements',
    'auto', 'insurance', 'accident', 'property', 'damage', 'billing',
    'statement', 'calculations', 'additional', 'enhance', 'submit'
  ];
  const domainTermsFound = insuranceDomainTerms.filter(term => responseLower.includes(term));
  const domainAccuracy = insuranceDomainTerms.length > 0 ? domainTermsFound.length / insuranceDomainTerms.length : 0;
  
  // Check for professional tone indicators (appropriate for PolicyHolder agent)
