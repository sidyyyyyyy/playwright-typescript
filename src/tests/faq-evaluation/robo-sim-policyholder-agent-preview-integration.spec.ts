...
import { 
  ...
  policyHolderAgentConfiguration,
  deleteAgent
} from '../../utils/robo-sim-agent-preview-helpers';
import * as fs from 'fs';
import * as path from 'path';

test.describe('RoboSim PolicyHolder Agent Preview Integration', () => {
  const testData = agentPreviewTestData;

  test('RoboSim with PolicyHolder Agent Preview - Address Update Flow', async ({ page, browser }) => {
    ...
    console.log('Starting RoboSim PolicyHolder Agent Preview Integration...');
    
    // Step 0: Create a new PolicyHolder agent for testing
    console.log('\nSTEP 0: Creating New PolicyHolder Agent for Testing');
    let createdAgentId = '';
    let createdAgentUrl = '';
    ...
    try {
      ...
      // Set environment variables for PolicyHolder agent creation
      ...
      console.log('Environment variables set for PolicyHolder agent creation');
      ...
      // Create a new agent with PolicyHolder configuration
      createdAgentUrl = await createAndActivateAgent(instance, token, agentConfig, account, policyHolderAgentConfiguration);
      console.log('Created PolicyHolder agent →', createdAgentUrl);
      ...
      console.log('Error creating PolicyHolder agent:', error);
      throw error;
    }
    ...
    // Step 4: Find and click on the specific PolicyHolder agent by ID
    console.log('\nSTEP 4: Finding PolicyHolder Agent by ID');
    ...
    // Look for the specific PolicyHolder agent by ID
    for (let i = 0; i < rowCount; i++) {
      ...
      // Look for PolicyHolder agents that are Active
      if (rowText && rowText.includes('PolicyHolder') && rowText.includes('Active')) {
        // Extract agent ID from the row text (usually the first part)
        const agentIdMatch = rowText.match(/^(\d+)/);
        if (agentIdMatch) {
          foundAgentId = agentIdMatch[1];
          console.log(`Found PolicyHolder agent with ID: ${foundAgentId}`);
    ...
    // If no PolicyHolder agent found, try to find by the created agent ID
    ...
    if (!selectedAgentRow) {
      throw new Error(`No PolicyHolder agent found in the agents table. Created agent ID: ${createdAgentId}`);
    }
    
    // Click on the selected agent row to open it
    console.log(`Clicking on PolicyHolder agent (ID: ${foundAgentId})...`);
      ...
      if (!uploadedFile) {
        ...
        const sampleApiSpec = {
          ...
          "info": {
            ...
            "version": "1.0.0",
            "description": "API for PolicyHolder Member Management"
          },
          "paths": {
            "/api/address/update": {
              "post": {
    ...
    if (await chatIframe.count() > 0) {
      ...
      if (chatFrame) {
        ...
        console.log('\n===== ROBOSIM POLICYHOLDER AGENT PREVIEW INTEGRATION COMPLETE =====');
    ...
    if (createdAgentId) {
      ...
      try {
        ...
        // Alternative: search by looking for PolicyHolder type agents
        if (!agentRow) {
          for (let i = 0; i < rowCount; i++) {
            const row = agentRows.nth(i);
            const rowText = await row.textContent();
            ...
            if (rowText && rowText.includes('PolicyHolder') && rowText.includes('Active')) {
              agentRow = row;
              console.log(`Found PolicyHolder agent in row ${i + 1}: ${rowText?.trim()}`);
              break;
            }
          }
        }
